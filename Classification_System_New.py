...
if daub_check == "epb" and epb_from_tests == 1:
    ...
    output = 0
    # txt file with value 0 in it means TBM type is EPB
    ...
    output += 1
    # txt file with value 0 in it means TBM type is Mixed Shield
else:
    print("\nSorry, but manual selection may be required. See Classification Report.txt for further information.")


filename = "grasshopper results from classification system.txt"
with open(filename, "w") as file:
    file.write(f"{output}")

# Work on providing user with full report of all values, so manual TBM determination can be conducted if needed, called
# Classification Report.txt. Essentially, this will show the user the graphs produced for inspection, values determined
# for cross-examination with laboratory results, and the ranges for which specific values were both satisfied and not
# satisfied.
